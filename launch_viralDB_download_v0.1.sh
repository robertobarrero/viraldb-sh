...
# -----------------------------
PIPELINE_DIR="${HOME}/git/viraldb-sh"
...
# Load config + utils
# -----------------------------
CONFIG_FILE="${PIPELINE_DIR}/config/config_viralDB.txt"
source "${CONFIG_FILE}"
...
for ID in "${IDENTITIES[@]}"; do
  ...
  # Map to your human label used in filenames
  if [[ "${ID}" == "1.0" ]]; then
    LABEL="c1000"
  elif [[ "${ID}" == "0.995" ]]; then
    LABEL="c995"
  ...
  CLSTR_FILE="${OUTDIR}/reps_all/all_groups/ALL/${CDIR}/reps.fasta.clstr"
  ...
  log "🔹🔹🔹 Summarising ${LABEL} using ${CLSTR_FILE}"
